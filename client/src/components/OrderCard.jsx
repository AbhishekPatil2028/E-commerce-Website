                ...
                userOrder?.map((order)=>(
                    <div key={order._id} className='shadow-lg rounded-2xl p-5 border border-gray-200'>
                        ...
                        <span className={`${order.status === "Paid" ? "bg-green-500":order.status === 
                            "Failed"? "bg-red-500":"bg-orange-300"} text-white px-2 py-1 rounded-lg`}>{order.status}</span>
                        </div>
                        ...
                        <ul className='space-y-2'>
                           {
                               order.products.map((product,index)=>(
                                ...
                                <img onClick={()=>Navigate(`/products/${product?.productId?._id}`)} className='
                                w-16 cursor-pointer' src={product.productId?.
                                    productImg?.[0].url} alt=""/>
                                <span className='w-[300px] line-clamp-1 '>{product.productId?.productName}</span>
                                
                                <span>{product?.productId?._id}</span>
                                <span className='font-medium'>
                                    ₹{product.productId?.productPrice} × {product.quantity} </span>
                                </li>
                            ))
                           }
                        </ul>
